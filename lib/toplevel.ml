...
let dump_to_object ~the_fpm =
  ...
  Llvm_target.TargetMachine.emit_to_file Codegen.the_module file_type filename
    machine;
  (* printf "Wrote %s\n" filename; *)
  ()
  ...
  let rec run_loop the_fpm the_execution_engine supplier =
    ...
    (try
       ...
       | `Expr ast ->
           ...
           let _tmp_name =
             Printf.sprintf "__toplevel%d" !anonymous_func_count
           in
...
let main input =
  ...
  | `File file ->
      In_channel.with_open_text file (run_main ~the_execution_engine ~the_fpm)

...
let rec codegen_expr = function
  | Ast.Expr.Var (var_names, body) ->
    ...
    List.iter var_names ~f:(fun (var_name, init) ->
      ...
      (match Hashtbl.find named_values var_name with
       | None -> ()
       | Some old_value -> old_bindings := (var_name, old_value) :: !old_bindings);
  ...
  | Ast.Expr.Call (callee_name, args) ->
    ...
    let callee =
      ...
      | Some callee -> callee
      | None -> raise_s [%message "undefined function" (callee_name : string)]
    in
    (* If argument mismatch error. *)
    if Int.( = ) (Array.length (Llvm.params callee)) (List.length args)
    then ()
  ...
  | Ast.Expr.For (var_name, start, end_, step, body) ->
    ...
    (* Convert condition to a bool by comparing equal to 0.0. *)
    let zero = Llvm.const_float double_type 0.0 in
    let end_cond = Llvm.build_fcmp Llvm.Fcmp.One end_cond zero "loopcond" builder in
  ...
  | Ast.Prototype (name, args) | Ast.BinOpPrototype (name, args, _) ->
    ...
    let f, existing =
      ...
      | Some f ->
        ...
        if Int.(Array.length (Llvm.params f) = List.length args)
        then ()
        else
          raise_s
            [%message
              "redefinition of function with a different number of args" (name : string)];
        f, `Not_existing
    in
    ...
    (try
     ...
     | e ->
       (match existing with
        ...
        | `Existing -> Array.iter (Llvm.basic_blocks the_function) ~f:Llvm.delete_block);
       raise e)
;;
